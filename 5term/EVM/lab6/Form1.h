...
private:
	...
	void draw_y(int i_t)
	{
		...
		for(int i = 0; i+n/40. < n; i+=n/40.)
		{
			...
			g->DrawLine(Pens::Gray, (int)(c->X + (double)2*m*10/max), c->Y - 6*m*10/may, c->X + 2*m*10/max, c->Y );
		pic->Image = canvas;
	}
	void expl(double eps)
	{
		if(x)
			delete x;
		if(y)
		...
		if(y2)
			delete y2;
		h = eps;
		tau = h*h/2;		//из устойчивости
		double alpha = tau/(2*h*h);
		n = 2/h + 1;
		n_t = 1/tau + 1;
		/*if(n < 7)
		{
		...
		for(int i = 1; i < n_t; i++)
			for(int j = 1; j < n-1; j++)
				y[i][j] = alpha*(y[i-1][j-1] + y[i-1][j+1]) + (1-2*alpha)*y[i-1][j] + tau*(tau*i*h*j);
	...
	void impl(double eps, double delta)
	{
		...
		for(int j = 1; j < n_t; j++)
		{
			...
			//y[j][1] = alpha/(1+2*alpha)*(y[j][0] + y[j][2]) + y[j-1][1]/(1+2*alpha) + h*h*h*1*j/(1+2*alpha);
		}
		if(checkBox1->Checked == true)
		{
			String^ s = "INTERFACE_PLOT3D(GRID(0. .. 2.,0. .. 1.,Array(1.." + n + ", 1.." + n_t +", [";
	...
	void button1_Click(System::Object^  sender, System::EventArgs^  e) 
	{
		...
		draw_coord();
		expl(Convert::ToDouble(epsilon->Value.ToString()));
		draw_coord();
	}
