...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			// button2
			// 
			this->button2->Location = System::Drawing::Point(557, 12);
...
private:
	...
	String^ dbl(double d)
	{
		String^s; 
		long long k = d;
		long long p = Math::Abs((d-k)*100000);
		s = "" + k + ".";
		if(p < 10)
			s += "0000";
		else if(p < 100)
	...
	void draw_y(int i_t)
	{
		...
		{
			g->DrawLine(Pens::Red, (int)(c->X + (double)h*(i)*m*10/max), c->Y - y[i][i_t]*m*10/may, c->X+ h*(i+n/40)*m*10/max, c->Y - y[i+n/40][i_t]*m*10/may);
		}
		else
			for(int i = 0; i < n-1; i++)
			{
				g->DrawLine(Pens::Red, (int)(c->X + (double)h*(i)*m*10/max), c->Y - y[i][i_t]*m*10/may, c->X+ h*(i+1)*m*10/max, c->Y - y[i+1][i_t]*m*10/may);
			}
	...
	void impl(double eps, double delta)
	{
		...
		//y = new double*[n];
		y = gcnew array<array<double>^>(n);
		t = gcnew array<double>(n);
		//double y[100][100] = {0};
		for(int i = 0; i < n; i++)
			y[i] = gcnew array<double>(n_t);
		for(int i = 0; i < n; i++)
			for(int j = 0; j < n_t; j++)
				y[i][j] = 0;
		for(int i = 0; i < n; i++)
		...
		for(int k = 0; k < 1000; k++)
			for(int i = 1; i < n-1; i++)
				for(int j = 1; j < n_t-1; j++)
					y[i][j] = (1./(2+2*alpha))*(y[i-1][j] + alpha*(y[i][j+1] + y[i][j-1]) + y[i+1][j] - h*h*Math::Sin(i*j*tau*h));						
			...
			String^ s = "INTERFACE_PLOT3D(GRID(0. .. 2.,0. .. 1.,Array(1.." + n + ", 1.." + n_t +", [";
				...
				for(int i = 0; i < n-1; i++)
				{
					...
					for(int j = 0; j < n_t-1; j++)
						s += "" + dbl(evalf(y[i][j])) + ", ";
					s += "" + dbl(evalf(y[i][n_t-1]));
						s += "],\n";
				}
				s+= "[";
				for(int j = 0; j < n_t-1; j++)
					s += "" + dbl(evalf(y[n-1][j])) + ", ";
				s += "" + dbl(evalf(y[n-1][n_t-1]));
					s += "]\n";
