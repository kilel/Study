...
void qr(double**a, double*b, int n)
{
	...
	for(int j = 0; j < n; j++)
		printf("x[%d] = %.6lf\n", j+1, b[j]);
	b = matrix_mult(a2,b,n);
	printf("\n");
	for(int j = 0; j < n; j++)
		printf("b[%d] = %.6lf\n", j+1, b[j]);
	
}
void maaaaax(double** a, int n, int &x, int&y)
{
	double max = a[1][0];
	x = 1;
	y = 0;
...
void razen_no_matrix(double**a, int n, double phi, int x, int y)
{
	for(int i = 0; i<n; i++)
		for(int j = 0; j < n; j++)
		{
...
void find_eign(double** a, int n, int print_val, int print_vect)
{
	print_matrix_matrix(a,a,n,n);
	double **b = new double*[n], ***u = new double**[2], max;
	double phi = 0;
	int x,y;
	u[0] = new double*[n];
	u[1] = new double*[n];
	...
	{
		maaaaax(a,n,x,y);
		phi = 2*a[x][y]/(a[x][x] - a[y][y]);
		if(fabs(a[x][x] - a[y][y]) > eps)
			phi = 1./2*atan(phi);
		else 
			phi = 3.14159265358979323846264338327/4;
		razen_no_matrix(u[0],n,phi,x,y);
		a = matrix_mult(transpose(u[0],n),matrix_mult(a,u[0],n),n);
		u[1] = matrix_mult(u[1], u[0],n);
		max = 0;
		maaaaax(a,n,x,y);
	
	
	}while(fabs(a[x][y]) > eps);
	print_matrix_matrix(a,a,n,n);
	if(print_val)
	{
		printf("values: \n");
