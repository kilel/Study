...
{			//функция вывода элементов каталога way2
	...
	{		//при двойном клике по любому списку
		...
		else			//если выбрана директория
		{
			...
			if(SetCurrentDirectory(GetFullPathName(index)))
				return;
		...
		if(s[1] == 'f')		//работа с файлом
		{
			...
			case Windows::Forms::DialogResult::Yes: 
				...
				case Windows::Forms::DialogResult::No: 
					{
						change_list1(sender,e);		//замысловато. выше не было break, поэтому сюда спустится
						change_list2(sender,e);
						return;
					}
			}
	...
	{			//создаёт новый файл/каталог
		...
		{
			if(7 == t)			//каталог
				Directory::CreateDirectory(s->Concat(Environment::CurrentDirectory, s, "\\"));
			else				//файл
			{
