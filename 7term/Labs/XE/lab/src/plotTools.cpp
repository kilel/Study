...
void plotTools::drawCurves(){
	...
	for(int i = 0; i < curve->Length; i++){
		if(curve[i]->Length > 10000){
			temp->Resize(temp,1000);
			for(int j = 0; j < temp->Length; j++)
				temp[j] = getPointFromReal( curve[i][j*(curve[i]->Length/1000.)] );
		}
		else{
			temp->Resize(temp,curve[i]->Length);
...
void plotTools::drawLines(){
	...
	for(int i = 0; i < line->Length; i++){
		temp->Resize(temp,line[i]->Length);
		for(int j = 0; j < line[i]->Length; j++)
			temp[j] = getPointFromReal( line[i][j] );
		g->DrawCurve(pensLine[i], temp);
	}
	delete temp;
}
...
void plotTools::addCurve(Line ^a, System::Drawing::Pen ^p){
...
void plotTools::addLine(Line ^a, System::Drawing::Pen ^p){
