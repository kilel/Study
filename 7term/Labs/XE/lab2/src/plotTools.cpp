...
void plotTools::drawLines(){
	...
	for(int i = 0; i < line->Length; i++){
		if(limLime[i] > 1000){
			temp->Resize(temp,100);
			for(int j = 0; j < temp->Length; j++)
				temp[j] = getPointFromReal( line[i][j*(limLime[i]/100.)] );
		}
		else{
			temp->Resize(temp,limLime[i]);
			for(int j = 0; j < limLime[i]; j++)
...
void plotTools::addLine(Line ^a, System::Drawing::Pen ^p, int lim){
	...
	limLime->Resize(limLime, line->Length);
	limLime[line->Length-1] = lim;
